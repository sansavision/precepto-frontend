...
export const AppSidebar: React.FC<AppSidebarProps> = ({ isShowSidebar, setActiveMenuItem, activeMenuItemTitle }) => {
    ...
    const [showSidebar, setShowSidebar ] = useGlobalStore(useShallow((state) => [state.showSidebar, state.setShowSidebar]));
            ...
            <div className="flex flex-col h-full">
                ...
                <div className={cx("flex flex-col flex-1 overflow-y-auto gap-8", !isShowSidebar ? "px-1 py-1" : "px-4 py-4")}>
                    ...
                    <nav className="flex flex-col gap-4 w-full">
                        {navItems.map((item) => {   
                            const count = getCount(transcriptions ?? [], item.status)
                            return (
                                <Button
                                    variant={"ghost"}
                                    type="button"
